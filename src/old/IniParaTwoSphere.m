...
%
%   Routine IniParaTwoSphere called by Biocad,
%
...
function tlm=IniParaTwoSphere(tlm)

% Directories Configuration

tlm.conf.src='C:\Documents and Settings\Vincent Senez\Mes documents\simulation\Biocad_2.1\src';
tlm.conf.result='C:\Documents and Settings\Vincent Senez\Mes documents\simulation\Biocad_2.1\result\twosphere';
...
if tlm.conf.dim==2
    ...
    tlm.var.RayonXMitoc(2)=0.50e-6/2;       % Radius X of the Mitochondria of the 2st Cell
    ...
    tlm.var.EpaisseurMitochon=1e-9;      % Thickness of the mitocho membrane

    tlm.var.DecentrageXCellule(1)=-5e-6;  % Lateral X location of the 1st cell by reference to tlm.var.OrigineX
    ...
    tlm.var.DecentrageXCellule(2)=5e-6;  % Lateral X location of the 1st cell by reference to tlm.var.OrigineX
...
elseif tlm.conf.dim==3
    ...
    tlm.var.RayonXCellule(1)=2.e-6;    % Radius X of the 1st Cell
    tlm.var.RayonYCellule(1)=2.e-6;    % Radius Y of the 1st Cell
    tlm.var.RayonZCellule(1)=2.e-6;    % Radius Z of the 1st Cell

    tlm.var.RayonXCellule(2)=2.e-6;    % Radius X of the 2nd Cell
    tlm.var.RayonYCellule(2)=2.e-6;    % Radius Y of the 2nd Cell
    tlm.var.RayonZCellule(2)=2.e-6;    % Radius Z of the 2nd Cell

    tlm.var.sha0=0.7;                 % 1st Parameter for the parametrization of the cell shape
    tlm.var.sha1=0;                 % 2nd Parameter for the parametrization of the cell shape

    tlm.var.RayonXNoyau(1)=1.0e-6/2;       % Radius X of the Nucleus of the 1st Cell
    tlm.var.RayonYNoyau(1)=1.0e-6/2;       % Radius Y of the Nucleus of the 1st Cell
    tlm.var.RayonZNoyau(1)=1.0e-6/2;       % Radius Z of the Nucleus of the 1st Cell

    tlm.var.RayonXNoyau(2)=1e-6/2;       % Radius X of the Nucleus of the 2st Cell
    tlm.var.RayonYNoyau(2)=1e-6/2;       % Radius Y of the Nucleus of the 2st Cell
    tlm.var.RayonZNoyau(2)=1e-6/2;       % Radius Z of the Nucleus of the 2st Cell
    ...
    tlm.var.DecentrageXCellule(1)=2.25e-6;  % Lateral X location of the 1st cell by reference to tlm.var.OrigineX
    tlm.var.DecentrageYCellule(1)=0e-6;    % Lateral Y location of the 1st cell by reference to tlm.var.OrigineY
    tlm.var.DecentrageZCellule(1)=5.0e-6;  % Vertical Z location of the 1st cell by reference to tlm.var.OrigineZ

    tlm.var.DecentrageXCellule(2)=-2.25e-6;  % Lateral X location of the 1st cell by reference to tlm.var.OrigineX
    tlm.var.DecentrageYCellule(2)=0e-6;     % Lateral Y location of the 1st cell by reference to tlm.var.OrigineY
    tlm.var.DecentrageZCellule(2)=5e-6;     % Vertical Z location of the 1st cell by reference to tlm.var.OrigineZ
...
% Physical Parameters
    ...
    tlm.conf.eps0=8.854187817e-012;     % Dielectric Permittivity of Vacuum
...
% Configuration of the Bio-Physical System
    ...
    tlm.conf.Dim=2;                     % 2 is for two dimensional analysis, 3 is for three D
    tlm.conf.Cell=2;                    % 1 if with one cell, 2 with 2 cells, 0 without cell
    tlm.conf.Shape=0;                   % 0 spheroid cell, 1 non_spheroid cell
    tlm.conf.Nucleus=0;                 % 1 if with a nucleus, 0 without a nucleus
    tlm.conf.Mitocho=0;                 % 1 if with a mitochondria, 0 without a mitochondria
    tlm.conf.Parasite=0;                % 1 if we take into account the hydratation layer around the electrodes
