...
%
%   Routine IniParaOneSphere called by Biocad,
%
...
function tlm=IniParaOneSphere(tlm)
...
%    tlm.var.RayonYMitoc(1)=0.25e-6/2;       % Radius Y of the Mitochondria of the 1st Cell
%    tlm.var.RayonZMitoc(1)=0.25e-6/2;       % Radius Z of the Mitochondria of the 1st Cell
    tlm.var.RayonXMitoc(1)=2.00e-6/2;       % Radius X of the Mitochondria of the 1st Cell
    tlm.var.RayonYMitoc(1)=2.00e-6/2;       % Radius Y of the Mitochondria of the 1st Cell
    tlm.var.RayonZMitoc(1)=2.00e-6/2;       % Radius Z of the Mitochondria of the 1st Cell
...
if tlm.conf.points==2
    tlm.conf.result='C:\Documents and Settings\Vincent Senez\Mes documents\simulation\Biocad_2.1\result\onesphere\2points';
elseif tlm.conf.points==4
    tlm.conf.result='C:\Documents and Settings\Vincent Senez\Mes documents\simulation\Biocad_2.1\result\onesphere\4points';
end
...
elseif tlm.conf.Test==29         %2D case - without cell - Two mediums are resistive & capacitive
                                %refine =0

% Name of the output files

    tlm.conf.Name = 'test29';

% Configuration of the Bio-Physical System

    tlm.conf.Dim=2;                     % 2 is for two dimensional analysis, 3 is for three D
    tlm.conf.Milo=2;                    % 1 is for one Medium, 2 is for two mediums
    tlm.conf.Cell=0;                    % 1 if with one cell, 2 with 2 cells, 0 without cell
    tlm.conf.Shape=0;                   % 0 spheroid cell, 1 non_spheroid cell
    tlm.conf.Nucleus=0;                 % 1 if with a nucleus, 0 without a nucleus
    tlm.conf.Mitocho=0;                 % 1 if with a mitochondria, 0 without a mitochondria
...
elseif tlm.conf.Test==30        %3D case - without cell - Two mediums are resistive & capacitive
                                %refine =0

% Name of the output files

    tlm.conf.Name = 'test30';
